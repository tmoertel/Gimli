...
-- Customized by Tom Moertel to allow for stacked prefix and postfix operators
    ...
    where
      makeParser term ops
        = let (rassoc,lassoc,nassoc
              ...
              termP      = do{ pres  <- prefixP
                             ; x     <- term
                             ; posts <- postfixP
                             ; return (foldr (.) id (reverse posts ++ pres) x)
                             }

              postfixP   = many postfixOp

              prefixP    = many prefixOp
